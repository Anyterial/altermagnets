...
def test_publish_uses_absolute_optimade_base_and_no_legacy_live_tables(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("ALTERMAGNETS_OPTIMADE_BASE_URL", "https://api.example.org/optimade")
    ...
    assert _widget_config(search, "httk-serve-optimade-table")["base_url"] == "https://api.example.org/optimade"
    assert _widget_config(material, "site-material-detail")["base_url"] == "https://api.example.org/optimade"
